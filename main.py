...
import pickle as pk#将对象以文件的形式存在磁盘上
...
def increase_pred_input_nd(num, df_lim: pd.DataFrame, data_set: int):
    ...
    """
    df_lim_dataset = df_lim.loc[df_lim["DATASET"] == data_set]#显示索引：.loc,第一个参数为 index切片，第二个为 columns列名
...
def dump_and_load_model(m):
    ...
    if os.path.exists(filename):
        ...
        with open(filename, "wb") as _f:
            ...
            #m.optimize_restarts() file generate_data.py has this line 
            pk.dump(m, _f)
            return m
...
def plot(xy, z, xy_pred, z_pred, n_dim):
    ...
    if xy_pred is not None:
        ...
        #ax.plot3D(xy_pred[:, 0], xy_pred[:, 1], z_pred[:, n_dim],'gray')
    plt.legend(loc='upper left')
    ax.set_xlabel('Longitufde Label')
    ax.set_ylabel('Latitude Label')
    ax.set_zlabel('RSSI Label')
    plt.show()
